...
Der er yderligere beskrevet indholdet af funktionen \textit{"Process\_Measurements"} vha. UML-aktivitetsdiagram. Til slut er den overordnet rækkefølge af programmets kodeeksekvering udført i et sekvens diagram.

Til slut vises et design af Matlab GUI med tilhørende objekter, såsom knapper, tekst og graf, hvilket sundhedspersonalet kan interagere med. 
...
\begin{tabularx}{\linewidth}{l  X  X}
     ...
     Start\_GUI	&		&	Set Date(now)\\   \addlinespace[2mm]
     			&		&	Set Billede\\   \addlinespace[2mm]
     	     	&		&	Set "Save Measurements"\\   \addlinespace[2mm]\hline\addlinespace[2mm]
	Generate\_SineWave   	&		&	Analog Discovery session (handles)\\   \addlinespace[2mm]\hline\addlinespace[2mm]
Read\_Measurements	&	Analog Discovery session (handles)	&	Message boks (Measurements running…)\\   \addlinespace[2mm]
			    &		&	data (handles)\\   \addlinespace[2mm]
			    &		&	timestamps (handles)\\   \addlinespace[2mm]\hline\addlinespace[2mm]
Process\_Measurements    &	data (handles)	&	BIsignal (handles)\\   \addlinespace[2mm]\hline\addlinespace[2mm]
Save\_Measurements    &	BIsignal (handles), Data (handles) og timestamps (handles)	&	CSV-fil\\   \addlinespace[2mm]
			    &		&	Message boks (Saving measurements...)\\   \addlinespace[2mm]
			    &		&	Message boks (Measurements saved!)\\   \addlinespace[2mm]\hline\addlinespace[2mm]
   		Show\_Measurements	    &	BIsignal (handles), Data (handles) og timestamps (handles)	&	Subplot measurements\\   \addlinespace[2mm]
     \bottomrule                                                                                                                   
    \end{tabularx}
    \caption {Tabellen viser funktionernes input og output parametre.}
    \label{tab:sw1}
...
{Figure/designUML}}
\caption{Figuren viser indholdet af kodeeksekvering i funktionen \textit{"Process\_Measurements"} hvor der udføres envelope af det rå BI signal. Først bliver det rå BI signal dobbeltensrettet og dernæst filtreret vha. digital lavpasfilter med knækfrekvens = 500 Hz og dæmper med 40 dB over en dekade.}
\label{Fig:designUML}
\end{figure} 
...
Når målingen er påbegyndt, vil der komme en massages boks frem som i figur \ref{Fig:designmessageboks1}.

\begin{figure}[H]
\centering
{\includegraphics[width=6cm]
{Figure/designmessageboks1}}
\caption{Massages boksen der kommer frem under målingerne.}
\label{Fig:designmessageboks1}
\end{figure} 

 Denne vil være fremme under hele målingen og informere om at en måling er i gang. Efter endt måling forsvinder massages boksen igen og målingen vises i to grafer, BI og EMG graf. Det er nu muligt at trykke på den anden knap \textit{"Btn\_Save\_Measurements"} for at gemme målingerne. Da målingen fylder meget vil gemme funktionen tage lidt tid. Derfor kommer massages boksen \ref{Fig:designmessageboks2} frem mens denne proces er i gang.
 
 \begin{figure}[H]
\centering
{\includegraphics[width=6cm]
{Figure/designmessageboks2}}
\caption{Massages boksen der informere om at målingerne er ved at blive gemt.}
\label{Fig:designmessageboks2}
\end{figure} 
 
 
 
  Når målingen er færdig kommer den sidste massages boks frem, figur \ref{Fig:designmessageboks3} for at informere om at målingerne nu er gemt i csv-fil.
